...
- 业务 `status ∈ {in_library, archived, recycle, deleted}`（4 值，任意可切，DB 行只增不改即"数据永生"）+ 文件 `file_state ∈ {present, missing, absent_confirmed}`（3 态，扫描 + 销毁操作维护）的双字段模型。
...
| 字段 | 取值 | 由谁维护 | 谁可改 |
|---|---|---|---|
| `status` | `in_library` / `archived` / `recycle` / `deleted` | 用户 | **任意来源的任意 status 可切到任意 status** |
...
│   ├── state_machine.rs # 状态机：DB 优先 + 文件 best-effort；任意 status→任意 status；目标目录同名视为孤儿（dirty_data）
...
├── covers/               # 抽取的封面（webp 字节，自定义后缀 .pwb 防止被看图软件收编；HTTP 按 magic bytes 探测 mime）
...
2. BLAKE3 算源文件 hash
3. 命中已存在行（同 hash）：
   - 行 `status='in_library'` → 删 inbox 副本，刷 `filename`（避免 dirty_scanner 把原 identified 副本当孤儿）
   - 行 `status` ∈ {`archived`, `recycle`, `deleted`} → `reactivate_row`：把源文件 mv 回 `doujinshi-identified/`、`status → in_library`；`deleted` 行也能复活
...
7. 落盘到 `doujinshi-identified/`、插 `doujinshi_file` 行（`status='in_library'`、`file_state='present'`）、写 `scan_event`
...
`MIGRATIONS` 数组是唯一真相，新增列或表 **必须 append 一条**，不要改旧条目。`CURRENT_VERSION=12`。

- v1 = `init_schema` bootstrap
- v2+ = `ALTER TABLE ADD/DROP COLUMN` 或 `CREATE TABLE IF NOT EXISTS`
- `apply_migration` 对 `ALTER TABLE ADD/DROP COLUMN` 走 `pragma_table_info` 幂等检查（兼容老库已有该列的情况），多语句 body 直接 execute 兜底
- 所有 migration 非破坏性：列带 `DEFAULT`，`CREATE TABLE IF NOT EXISTS`，已有数据零损耗
- 当前历史 schema 演进：v1 bootstrap → v2/v3 加列 → v4 加 `has_physical_file`（V0.2.0 v11 已 DROP） → v5 dirty_data → v6 折 `physically_deleted` → v7 封面 .pwb → v8 重命名 `current_location/path` 为 `status/last_seen_path` + 加 `file_state` → v9 DROP `version_tag` → v10 加 `dirty_data.resolved_at` → v11 DROP `has_physical_file` → v12 DROP `filename_alias`
...
| `/api/doujinshi/:id/archive` | POST | 任意 status → `archived` |
| `/api/doujinshi/:id/restore` | POST | 任意 status → `in_library`（含 deleted→in_library） |
...
- `useLibraryStore`：`items` / `queryInput`（v-model 用） / `query`（防抖 300ms 后真正查询） / `status`（all/viewed/not_viewed/marked） / `statusFilter`（active/all/in_library/archived/recycle/deleted，默认 active=排除 recycle+deleted） / `topCircles`（top 10 社团） / `visibleItems`（active 模式下二次过滤）
- `useRecycleStore`：`present`（只展示 `status='recycle' + file_state='present'` 的「待删除文件」，gone 段不在此处——对应记录可在 Library 用 status filter 找到）
...
- 封面输出是 webp（`cover_format::encode_webp`），HTTP `cover` handler 按 magic bytes 探测 mime 而不是看扩展名（早期 jpg 后缀文件也走该路由）。
...
按主题归档，不带版本号（避免 AI 接手时被「V 编号」误导成按时间线去读旧 spec）：

- 数据与文件解耦 spec：`docs/superpowers/specs/2026-07-15-decouple-data-and-file.md`（业务 status + 文件 file_state 双字段，当前权威）
- 数据备份与还原 spec：`docs/superpowers/specs/2026-07-15-data-backup.md`（`data.db` 本地备份/还原）
- 脏数据页「重新入库」spec：`docs/superpowers/specs/2026-07-16-dirty-reingest-button.md`（orphan_file → 搬到 inbox → scanner 接管）
- 文件回收站简化 spec：`docs/superpowers/specs/2026-07-16-v46-recycle-simplification.md`（RecycleBin 只展示 present）
- 设置页重设计 spec：`docs/superpowers/specs/2026-07-16-v47-settings-redesign.md`（卡片 + 字段统一 + 备份 mtime）
- HTTP API 测试弹窗 spec：`docs/superpowers/specs/2026-07-16-v48-api-test-dialog.md`（设置页路由表内联测试入口）
- 列表封面开关 spec：`docs/superpowers/specs/2026-07-16-v49-cover-toggle.md`（Inbox / RecycleBin 标题右侧 Image/Rows3 切换）
