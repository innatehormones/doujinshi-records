...
<template>
  <div class="page">
    <header class="flex items-baseline justify-between gap-4">
      ...
      <span class="font-mono text-caption text-smoke tracking-[0.1em]">{{ store.entries.length }} 条</span>
    </header>
    ...
    <n-spin :show="store.loading">
      ...
      <div v-else class="flex flex-col gap-2">
        <article v-for="e in store.entries" :key="e.id" class="flex items-start gap-4 rounded-cards border border-border bg-card p-4">
