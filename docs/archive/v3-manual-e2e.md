...
| E3 | 归档操作 | E2 之后 | Library 卡片点「归档」 | zip 移到 `doujinshi-archived/`；Library 筛选改「归档」能看到；「全部」视图筛掉该卡片；DB `status = 'archived'`；HTTP 状态 200 | 文件还在 identified/；按钮无反应；后端报错 |
| E4 | 移到回收站 | E2 之后 | 卡片点「移到回收站」 | zip 移到 `doujinshi-will-delete/`；Library 「回收站」筛选可见；DB status 切到 recycle | 文件还在；status 标签未切 |
| E5 | 回收站取回 | E4 之后 | RecycleBin 页点「取回」 | zip 移回 `doujinshi-identified/`；Library 全部 / 已入库可见 | 文件还在 will-delete；status 没切 |
| E6 | 彻底清理 | E4 之后 | RecycleBin 页点「彻底清理」 | zip 从 will-delete/ 删；DB 行 `file_state = 'absent_confirmed'` + `status = 'deleted'`；卡片显示「文件已销毁」红色「!」角标 | 文件没删；角标没出 |
...
| E8 | 同 hash 复活 | E6 之后 | 把 `doujinshi-archived/` 那个 zip 复制到 `doujinshi/`（文件名任意） | ~3s 后 Library 看到的是**同一行**（id 复用、`status = in_library`、hash 一致）；旧的 `file_state` 翻回 present | 新建了另一行（id 重复）；status 卡在 recycle |
...
V4（提交 `b12d81d..db97fdc`）后本矩阵的字段名/语义有更新：

- DB 字段：`current_location` → `status`（4 值 `in_library / archived / recycle / deleted`，任意可切）；`current_path` → `last_seen_path`；`has_physical_file` (bool) → `file_state` (TEXT, `present / missing / absent_confirmed`)
- 状态机：从"强一致"改为"DB 优先 + 文件 best-effort"——E6「彻底清理」现在不影响其他 status 切换；状态机允许任意 from→to（即便文件缺失也允许改 status）
- 目标目录同名：旧逻辑"拒绝转移"，V4 改为"自动覆盖 + 写 dirty_data(reason='overwritten_by_state_switch')"
- 销毁复合操作：E6 一次完成 `status=deleted` + `file_state=absent_confirmed` + best-effort 删文件 + preview_cache invalidate；不再走 state_machine
- 新的 V4 手动 E2E 矩阵参见 `docs/superpowers/plans/2026-07-15-decouple-data-and-file.md` §Task 15 step 4
